...
- [§2/§6] — `src/app/landowner/requests/[id]/page.tsx` — Farmer B's (landowner) guided lane was one handoff screen plus the shared workspace, with no standalone front door and no way to list his paddock's own capacity. — **partially fixed 14 Jul 2026**: added `src/app/landowner/page.tsx` (hub showing his paddock listing, the incoming request, and his agreements) and `src/app/landowner/listings/new/page.tsx` (list/edit paddock capacity: acres, stock accepted, water, fencing, rate — stored in `WorkflowState.landowner`, `src/lib/paddockmeWorkflow.tsx`). `GUIDED_DEMO_LANES.landowner` now points at the hub. Still debt: the hub links out to the existing single accept/decline screen and the shared workspace rather than a landowner-native review/negotiation flow of its own — a full mirror of the owner and carrier lanes' depth is a separate build brief.

## 20 Jul 2026 — workspace party-attribution fix (audit P1/P2)

Workspace viewer party no longer defaults to "A" (`WorkspaceClient.tsx` resolves the signed-in user against the agreement's stored party ids; unresolved/non-party users get a blocked state and no mutations). Section edits and agree ticks derive the caller's party in `repositories.ts` (`resolveCallerParty` via `src/lib/agreementParty.ts`) — client-supplied `party`/`agreedByA`/`agreedByB` inputs removed. DB-level enforcement: migration `20260720090000_agreement_section_party_enforcement.sql` (trigger blocking cross-party ticks/edits; verified against a disposable Postgres 16 with authenticated-role tests, idempotent). Unit coverage: `tests/unit/agreement-party.test.ts` (`npm run test:unit`).
