...
| Direct Supabase outside repository boundary (drift) | `src/app/(app)/listings/new/page.tsx` · `src/app/(app)/requests/RequestsClient.tsx` · `src/app/(app)/transport/available/CapacityClient.tsx` · `src/app/(auth)/onboarding/OnboardingClient.tsx` · `src/components/AppShellHeaderUser.tsx` · `src/lib/payments/ledger.ts` · auth pages | Auth checks, profile reads/upserts, transport capacity inserts, and payment ledger writes currently bypass the `src/lib/data/*` boundary. Logged in `SPEC_DRIFT.md`; consolidate under a future repository brief. |
